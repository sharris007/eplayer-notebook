...
const ACTION_HANDLERS = {
  ...
  [BOOKS_FULFILLED]: (state, action) => ({
    ...
    error: null }),
  [BOOK_DETAILS]: (state, action) => ({ ...state, authorName: action.authorName, title: action.title, thumbnail: action.thumbnail, globalUserId: action.globalUserId, globalBookId: action.globalBookId, bookeditionid: action.bookeditionid, uPdf: action.uPdf, serverDetails: action.serverDetails, bookId: action.bookId, uid: action.uid, ubd: action.ubd, ubsd: action.ubsd, roleTypeID: action.roleTypeID, firstName: action.firstName, lastName: action.lastName, expirationDate: action.expirationDate, userEmailId: action.userEmailId }),  // eslint-disable-line max-len
