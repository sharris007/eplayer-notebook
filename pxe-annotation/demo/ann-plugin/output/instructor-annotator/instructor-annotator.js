...
// Added to get absolute path ex:- 'p[@id]'
var getPreferredXPath = (function(){
  
  function getXPathWithAttr(elem, attr) {
    var tag = elem.tagName.toLowerCase();
    var tagValue = elem.getAttribute(attr);
    return "//" + tag + "[@"+attr+"='" + tagValue + "']";
  }

  return function (elem) {
    return getXPathWithAttr(elem, "id");
  };
})();
...
simpleXPathJQuery = function(relativeRoot) {
  ...
  jq = this.map(function() {
    ...
    while ((elem != null ? elem.nodeType : void 0) === Node.ELEMENT_NODE && elem !== relativeRoot) {
      var prefXpath = getPreferredXPath(elem);
      if (prefXpath) {
        return prefXpath+path;
      }
